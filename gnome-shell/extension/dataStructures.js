...
/**
 * MIME types that are considered image types.
 */
...
/**
 * Detect the content type from a MIME type string.
 *
 * Returns one of TYPE_TEXT, TYPE_IMAGE, or TYPE_BINARY.
 */
export function typeFromMime(mimeType) {
  if (!mimeType) {
    return TYPE_TEXT;
  }
...
/**
 * Return a human-readable placeholder label for non-text clipboard content.
 *
 * For image MIME types (image/png, image/jpeg, etc.) the label reads
 * "[Image: <mimeType>]".  For other binary MIME types the label reads
 * "[Binary data: <mimeType>]".  The label is truncated to MAX_VISIBLE_CHARS
 * characters so it fits within the menu item width.
 *
 * @param {string} type - one of TYPE_TEXT, TYPE_IMAGE, TYPE_BINARY
 * @param {string} [mimeType] - optional MIME type string
 * @returns {string}
 */

/** Maximum visible characters for a placeholder label (matches extension.js). */
export const MAX_VISIBLE_CHARS = 200;

/**
 * Truncate a string to at most maxLen characters, appending an ellipsis if
 * it was shortened.
 *
 * @param {string} s
 * @param {number} [maxLen]
 * @returns {string}
 */
export function truncateLabel(s, maxLen = MAX_VISIBLE_CHARS) {
  if (s.length <= maxLen) {
    return s;
  }
...
export function placeholderLabel(type, mimeType) {
  if (type === TYPE_IMAGE) {
    return truncateLabel(`[Image: ${mimeType ?? 'image'}]`);
  }
  if (type === TYPE_BINARY) {
    return truncateLabel(`[Binary data: ${mimeType ?? 'application/octet-stream'}]`);
  }
  return '';
}
