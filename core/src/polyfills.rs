...
impl<'a, T: Copy> BorrowedCursor<'a, T> {
    ...
    /// # Safety
    ///
    /// The caller must not uninitialize any elements of the cursor if it is
    /// initialized.
    #[inline]
    ...
    pub fn with_unfilled_buf<R>(&mut self, f: impl FnOnce(&mut BorrowedBuf<'_, T>) -> R) -> R {
        ...
        //
        // SAFETY: These elements were initialized/filled in the `BorrowedBuf`, and
        // therefore they are initialized/filled in the cursor too, because the
...
impl<'a> io::Write for BorrowedCursor<'a, u8> {
    #[inline]
