...
pub const unsafe fn as_uninit_slice_mut<'a, T>(
    ...
    if self_.is_null() {
        ...
        Some(unsafe { slice::from_raw_parts_mut(self_ as *mut MaybeUninit<T>, self_.len()) })
    }
}
...
impl<'data> From<&'data mut [u8]> for BorrowedBuf<'data> {
    ...
    fn from(slice: &'data mut [u8]) -> BorrowedBuf<'data> {
        ...
        BorrowedBuf {
            ...
            buf: unsafe { as_uninit_slice_mut(slice as *mut [u8]).unwrap() },
            filled: 0,
            init: len,
        }
    }
...
#[inline(always)]
pub const unsafe fn assume_init_ref<T>(self_: &[MaybeUninit<T>]) -> &[T] {
    ...
    // valid for reads.
    unsafe { &*(self_ as *const [MaybeUninit<T>] as *const [T]) }
}

#[inline(always)]
pub const unsafe fn assume_init_mut<T>(self_: &mut [MaybeUninit<T>]) -> &mut [T] {
    // SAFETY: similar to safety notes for `slice_get_ref`, but we have a
    // mutable reference which is also guaranteed to be valid for writes.
    unsafe { &mut *(self_ as *mut [MaybeUninit<T>] as *mut [T]) }
}
...
impl<'data> BorrowedBuf<'data> {
    ...
    /// # Safety
    ///
    /// The caller must ensure that the first `n` unfilled bytes of the buffer
...
/// inform the cursor how many bytes have been written.
///
...
pub const fn write_copy_of_slice<'a, T>(self_: &'a mut [MaybeUninit<T>], src: &[T]) -> &'a mut [T]
where
    T: Copy,
{
    ...
    self_.copy_from_slice(uninit_src);

    // SAFETY: Valid elements have just been copied into `self` so it is initialized
    unsafe { assume_init_mut(self_) }
}

impl<'a> BorrowedCursor<'a> {
    /// Reborrows this cursor by cloning it with a smaller lifetime.
    ///
    /// Since a cursor maintains unique access to its underlying buffer, the
    /// borrowed cursor is not accessible while the new cursor exists.
    #[inline]
    ...
    /// Returns the number of bytes written to the `BorrowedBuf` this cursor was
    /// created from.
    ///
    ...
    pub fn init_mut(&mut self) -> &mut [u8] {
        // SAFETY: We only slice the initialized part of the buffer, which is always
        // valid
        unsafe {
            let buf = self
                .buf
    ...
    pub fn ensure_init(&mut self) -> &mut Self {
        ...
        // SAFETY: 0 is a valid value for MaybeUninit<u8> and the length matches the
        // allocation since it is comes from a slice reference.
        unsafe {
    ...
    pub fn append(&mut self, buf: &[u8]) {
        ...
        unsafe {
            write_copy_of_slice(&mut self.as_mut()[..buf.len()], buf);
        }
